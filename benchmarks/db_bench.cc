...
// Number of key/values to place in database
static int FLAGS_batch_num = -1;

// Number of read operations to do.  If negative, do FLAGS_num reads.
static int FLAGS_reads = -1;

// Number of each scan operations. If negative, do 1000.
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      if (name == Slice("open")) {
        ...
        fresh_db = true;
        entries_per_batch_ = (FLAGS_batch_num < 0 ? 1000 : FLAGS_batch_num);
      ...
      } else if (name == Slice("scanseq")) {
        method = &Benchmark::ScanSeq;
      ...
      } else if (name == Slice("writetime")) {
        PrintStats("leveldb.writetime");
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_num = n;
    } else if (sscanf(argv[i], "--batch_num=%d%c", &n, &junk) == 1) {
      FLAGS_batch_num = n;
    } else if (sscanf(argv[i], "--reads=%d%c", &n, &junk) == 1) {
      FLAGS_reads = n;
    } else if (sscanf(argv[i], "--scan_length=%d%c", &n, &junk) == 1) {
      FLAGS_scanLength = n;
